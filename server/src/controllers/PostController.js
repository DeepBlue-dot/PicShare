...
export async function createPost(req, res) {
  ...
  user.toggleSavePost(newPost._id);
...
export async function searchPosts(req, res, next) {
  const { search, tags, createdBy, createdAtStart, createdAtEnd, sort } =
    req.query;
  ...
  const formattedPosts = posts.map((post) => ({
    ...
    saved: userId ? userSavedPosts.includes(post._id.toString()) : false,
    tags: post.tags || [],
